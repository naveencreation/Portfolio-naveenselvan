...
export function ActProjects({ projects }: ActProjectsProps) {
    ...
    useEffect(() => {
        if (!sectionRef.current || sortedProjects.length === 0) return;

        cardRefs.current = cardRefs.current.slice(0, sortedProjects.length);

        let ctx: ReturnType<typeof gsap.context> | null = null;

        // Wait one frame so layout is stable before GSAP reads positions
        const raf = requestAnimationFrame(() => {
            ctx = gsap.context(() => {
                ...
                // ─── Header: fade-in elements on page load / before pin ───
                ...
                // ─── MAIN PIN: pin the entire section ───
                // Scroll budget: each card transition gets 100vh of scroll
                ...
                const mainTl = gsap.timeline({
                    scrollTrigger: {
                        ...
                        pin: true,
                        pinSpacing: true,      // adds pin-spacer AFTER the section so downstream sections stay put
                        scrub: 0.6,            // light scrub = responsive but smooth
                        anticipatePin: 1,
                        onUpdate: (self) => {
                ...
                sortedProjects.forEach((_, idx) => {
                    ...
                    // Transition occupies 1 / numTransitions of total progress
                    ...
                    for (let prevIdx = 0; prevIdx < idx; prevIdx++) {
                        ...
                        // How deep is this card from the top after this transition?
                        const depth = idx - prevIdx;
                        const scaleVal = Math.max(0.88, 1 - depth * 0.04);
                        // Only 0-2 depths have visible opacity; deeper cards fade out completely
    ...
    return (
        <section
            ref={sectionRef}
            className="act-projects w-full bg-[var(--color-background)] overflow-hidden"
            style={{ minHeight: '100vh' }}
        >
            <div className="act-content max-w-5xl mx-auto w-full px-6 h-full flex flex-col justify-center py-16">

                {/* ── Act Header ── */}
                <div ref={headerRef} className="narrative-center mb-10">
                ...
                {/* ── Progress Dots ── */}
                <div ref={dotsRef} className="flex justify-center items-center gap-3 mt-8">
